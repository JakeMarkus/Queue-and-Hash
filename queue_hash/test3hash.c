...
int main()
{
	...
	printf("Removing a struct with name Trajan:\n");

	TestStruct_t* found = hremove(table, compare, name1, strlen(name1));


	printf("Removed Struct:\n");
  ...
  happly(table, printTestStruct);
	...
	printf("Removing a non-existent struct with name non-existent name: Marc Antony! \n");
	...
	printf("Hash table should be undisturbed (len 2):\n");
	happly(table, printTestStruct);
